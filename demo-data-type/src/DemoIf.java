public class DemoIf {
  public static void main(String[] args) {

    int x = 3;
    boolean result = x > 3;
    System.out.println(result);// false


    boolean result2 = x >= 3;
    System.out.println(result2);


    // if  條件
    if (x > 3) {// false -> skip code block
      System.err.println("x > 3, x is larger than 3");
      
    }
    if (x >= 3) {

      System.out.println("x >= 3, x is larger and equal to 3");
    }

    // if + else
    if (x > 3) {
      System.out.println("do something ...");
    } else { //implication: x <= 3}
      System.out.println("do something else ...");
    }


    // ==
    // x = 3, "=" -> assigment (assign 3 to x)
    // ! "==" -> asking if they are same value
    char gender = 'M';
    if (gender == 'M') {
      System.out.println("He is male");
    } else {
      System.out.println("She is female");
    }

    // "!=" -> asking if they are not equal
    if (gender != 'M'){
      System.out.println("Not a male");
    }

    if (x == 3){

      System.out.println("ohMyGodxIsEqualsTo3");
    }


    // Math Operators: +, -, *, /, %

    int r = 14 / 3;
    System.out.println(r); //4

    // "%" -> remainder
    int r2 = 14 % 3;
    System.out.println(r2);

    // Comparsion operator: >, <, >=, <=, !=, ==
    // Apply for all primitives


    //String ()
    //equals()
    String s2 = "Bootcamp";
    if (s2.equals("Bootcamp")){
      System.out.println("12345");
    }

    // Approach 1
    if (s2.equals("Bootcamp") == false){
      System.out.println("23456");
    }

    //Approach 2
    //Step 1: s2.equals("Bootscamp") -> true
    //Step 2: !true -> false
    //Step 3: Skip code Block
    if (!s2.equals("Bootcamp")){
      System.out.println("23456");
    }

    boolean r3 = !(x > 3); // ! can make true -> false or false -> true
    System.out.println(r3);

    boolean isRaining = true;

  if (!isRaining) {
    System.err.println("I gonna go for run ...");
    
  }
  //if + length(), charAt()
  // "s2.length() >= 3" -> true / false
  if (s2.length() >= 3){
    System.out.println("hello");//print
  } else {
    System.out.println("goodbye");//skip
  }
  
  // Question: check if the last character of s2 equals to 'a' 
  // if yes, print "yes", if no, print "no"
  
 //My Version
  if (s2.charAt(s2.length() - 1) == 'a') {
    
  System.out.println("yes");
  } else {
    System.out.println("no");
 //Vincent Version
 // ! charAt() + length()
 // Step 1: s2.length() -> 8
 // Step 2: 8 - 1 -> 7
 // Step 3: charAt(7)
 // Step 4: 'p' == 'a' -> false
 // Step 5: print no

// s2 = "";
 if (s2.charAt(s2.length() - 1) == 'a') {
  System.out.println("Yes");
} else {
  System.out.println("no");
}


 }


 //Question: Find the middle char of the string
 String s3 = "hello";
 //System.out.println(s3.charAt(s3.length() %2));
 System.out.println(s3.charAt(s3.length() /2  ));
  
 String l3 = "abcd";
if (s3.length() %2 == 1) { //odd length


}




// odd number
int x5 = 7;
// Assignment (right to left)
boolean isOdd = x5 %2 == 1;
System.out.println(isOdd); //true


boolean isLargerThan10 = x5 > 5;
System.out.println(isLargerThan10);//true
// && -> AND
if (isOdd && isLargerThan10) {

System.out.println("It is an odd number");

}

if (isOdd && isLargerThan10) {
  System.out.println("It is an even number and larger than 5.");
}

  if (x5 % 2 == 1 && x5 > 5);


  // || -> or
  if (isOdd || isLargerThan10) {
    System.out.println("It is an odd number OR larger than 5");
  }



  int x6 = 10;
  // Step 1: x6 < 9. if true, exit criteria checking; if false, go step 2.
  // Step 2: x6 % 2 == 0 && x6 > 0 event go first 
  if (x6 < 9 || x6 % 2 == 0 && x6 > 0){
    System.out.println("Yes");

  }



//  Count how many character 'o' in s5.
//  int counter = 0;
//  if (s5.charAt(0) == 'o'){
//    counter = counter + 1;
//  }

//  if (s5.charAt(1) == 'o'){
//    counter = counter + 1;
//  }

//  if (s5.charAt(2) == 'o'){
//    counter = counter + 1;
//  }

//  if (s5.charAt(3) == 'o'){
//    counter = counter + 1;
//  }

//  if (s5.charAt(0) == 'o'){
//    counter = counter + 1;
//  }

//  if (s5.charAt(0) == 'o'){
//    counter = counter + 1;
//  }

//  if (s5.charAt(0) == 'o'){
//    counter = counter + 1;
//  }

// !For Loop
// 3 statements
// for (initialization; continue condition; )
// Step 1: int i = 0;
// Step 2: check if i< 3 -> true, next setp
// Step 3: Print
// Step 4: i = i + 1 -> 1
// Step 5: check if i < 3 -> true. next step
// Step 6: print hello
// Step 7: i = i + 1 -> 2
// Step 8: check if i < 3 -> true. next step
// Step 9: print hello
// Step 10: i = i + 1 -> 3
// Step 11: check if i < 3 -> false -> exit the loop



    for (int i = 0; i < 3; i = i + 1){ //
      System.out.println("hello");

    }

  //2

    // Question: find out how many letter 'o' in the charater
    String s5 = "hello world";
    int counter = 0;
    for (int i = 0; i < s5.length() ; i = i + 1){
      if (s5.charAt(i) == 'o'){
        counter = counter + 1;
      }
    }
      System.out.println("Number of o =" + counter);


  }

  }
  

